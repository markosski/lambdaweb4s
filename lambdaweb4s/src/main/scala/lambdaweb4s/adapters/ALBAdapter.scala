...
class ALBAdapter(rawReq: String) extends RequestResponseAdapter[Array[Byte]]{
  private val mapper = new ObjectMapper() with ScalaObjectMapper
  mapper.registerModule(DefaultScalaModule)
  mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false)

  def parse: Either[Throwable, ALBRequest] = {
    Try {
      mapper.readValue[ALBRequest](rawReq)
    } match {
  ...
  def toJson(response: ALBResponse): Either[Throwable, Array[Byte]] = {
    Try {
